...
import {
  ...
  Check, X, Trash2, Plus, Server, TestTube, Shield, LogOut
} from 'lucide-react';
                ...
                {videos.map(v => (
                  <div key={v.id} className="bg-card rounded-xl border border-border/50 p-4 flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {v.status === 'pending' && (
                        <>
                          <Button size="sm" onClick={() => updateVideoStatus(v.id, 'approved')} className="bg-success text-success-foreground h-8">
                            <Check className="w-3.5 h-3.5" />
                          </Button>
                          <Button size="sm" onClick={() => updateVideoStatus(v.id, 'rejected')} variant="destructive" className="h-8">
